template <typename ND, typename T = typename ND::T> struct segment_tree {
  int n;
  vector<ND> seg;
  ...
  segment_tree(vector<T> &v) : n(v.size()), seg(4 * n) { build(1, 0, n - 1); }
...
struct node {
  ...
  node() : x(0) {}
  node(T _x) : x(_x) {}
  friend node operator+(node a, node b) { return node(a.x + b.x); }
};
